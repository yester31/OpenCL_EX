...
int main()
{
	...
	output = (float*)malloc(sizeof(float)* N * 1 * H * W);		// Ouput [N, 1, H, W] Y
	...
	buffer_O = clCreateBuffer(context, CL_MEM_WRITE_ONLY, sizeof(float) * N * 1 * H * W, NULL, &status);	// 디바이스 버퍼 객체 생성(출력용)
	...
	char* programSource = readSource("split.cl"); // 커널 함수 파일 로드
	...
	cl_kernel split_channel_kernel = NULL;
	// NCHW -> NHWC
	split_channel_kernel = clCreateKernel(program, "split_first_channel_kernel", &status); // 커널 생성 (커널 함수 이름을 인자로 전달)
	...
	status = clSetKernelArg(split_channel_kernel, 0, sizeof(cl_mem), &buffer_O); // output
	status = clSetKernelArg(split_channel_kernel, 1, sizeof(cl_mem), &buffer_I); // input
	status = clSetKernelArg(split_channel_kernel, 2, sizeof(cl_int), &num_elements); // 커널 함수 인자 값 전달 (버퍼와 커널 연결)
	...
	status = clEnqueueNDRangeKernel(cmdQueue, split_channel_kernel, 1, NULL, globalWorkSize, NULL, 0, NULL, &event); // 커널 실행
	...
	status = clEnqueueReadBuffer(cmdQueue, buffer_O, CL_TRUE, 0, sizeof(float) * N * 1 * H * W, output, 0, NULL, NULL); // device (bufferC) -> host (C) 전달

	//printData(output, N, C, H, W);
	//tofile(input, N * C * H * W, "../Validation_py/Input_C_4");
	tofile(output, N * 1 * H * W, "../Validation_py/Output_C_4");

	// python 검증 스크립트 수행
	printf("\n *Validation with python \n");
	std::string command = "python ../Validation_py/split_channel.py --N=" + std::to_string(N) + " --C=" + std::to_string(C) + " --H=" + std::to_string(H) + " --W=" + std::to_string(W);
	...
	clReleaseKernel(split_channel_kernel);
