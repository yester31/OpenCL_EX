...
void conv2d_opencl(cl_mem outputBuffer, cl_mem inputBuffer, cl_mem weightBuffer, Configs &ccfg, ocl_args_d_t &ocl, cl_int status, bool time = false)
{
	...
	status = clSetKernelArg(col2im_kernel, 2, sizeof(cl_int), &ccfg.N);
	status = clSetKernelArg(col2im_kernel, 3, sizeof(cl_int), &ccfg.K);
	status = clSetKernelArg(col2im_kernel, 4, sizeof(cl_int), &ccfg.P);
	status = clSetKernelArg(col2im_kernel, 5, sizeof(cl_int), &ccfg.Q);
	if (status != 0) checkError(status, __LINE__);
	//================================================================================================================
