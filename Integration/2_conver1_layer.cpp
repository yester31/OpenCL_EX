...
int main() {
	...
	//float *output = (float*)malloc(sizeof(float)* ccfg_b.N * ccfg_b.K * ccfg_b.P * ccfg_b.Q);		// bicubic Ouput [N, C, P, Q]
	float *output = (float*)malloc(sizeof(float)* ccfg_c1.N * ccfg_c1.K * ccfg_c1.P * ccfg_c1.Q);	// convert-1 Ouput [N, C, P, Q]
	//================================================================================================================
	...
	status = clEnqueueReadBuffer(ocl.commandQueue, outputBuffer_c1, CL_TRUE, 0, sizeof(float) * ccfg_c1.N * ccfg_c1.C * ccfg_c1.P * ccfg_c1.Q, output, 0, NULL, NULL); // device (bufferC) -> host (C) 전달
	if (status != 0) checkError(status, __LINE__);
	...
	if (0) {// 검증
		...
		tofile(output, ccfg_b.N * ccfg_b.C * ccfg_b.P * ccfg_b.Q, "../Validation_py/Output_I");
