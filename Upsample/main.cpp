...
int main() {
	...
	int h_scale = (int)rescale_factor;
	int w_scale = (int)rescale_factor;
	int num_elements = N * C * P * Q;
	printf("input[%4d,%4d,%4d,%4d] -> output[%4d,%4d,%4d,%4d]\n\n", N, C, H, W, N, C, P, Q);
	int flag = 0;// nearest neighbor
	// Host data
	// Allocate space for input/output data
	float *input = (float*)malloc(sizeof(float)* N * C * H * W);	// input [N, C, H, W]
	float *output = (float*)malloc(sizeof(float)* N * C * P * Q);	// Ouput [N, C, P, Q]
	...
	cl_mem buffer_I = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(float) * N * C * H * W, NULL, &status);	// 디바이스 버퍼 객체 생성(입력용)
	cl_mem buffer_O = clCreateBuffer(context, CL_MEM_WRITE_ONLY, sizeof(float) * N * C * P * Q, NULL, &status);// 디바이스 버퍼 객체 생성(출력용)
	...
	char* programSource = readSource("upsample.cl"); // 커널 함수 파일 로드
	...
	cl_kernel nearnest_neighbor_kernel = clCreateKernel(program, "nearnest_neighbor_kernel", &status); // 커널 생성 (커널 함수 이름을 인자로 전달)
	...
	status = clSetKernelArg(nearnest_neighbor_kernel, 0, sizeof(cl_mem), &buffer_O); // output
	status = clSetKernelArg(nearnest_neighbor_kernel, 1, sizeof(cl_mem), &buffer_I); // input
	status = clSetKernelArg(nearnest_neighbor_kernel, 2, sizeof(cl_int), &h_scale); // 커널 함수 인자 값 전달 (버퍼와 커널 연결)
	status = clSetKernelArg(nearnest_neighbor_kernel, 3, sizeof(cl_int), &w_scale); // 커널 함수 인자 값 전달 (버퍼와 커널 연결)
	status = clSetKernelArg(nearnest_neighbor_kernel, 4, sizeof(cl_int), &N); // 커널 함수 인자 값 전달 (버퍼와 커널 연결)
	status = clSetKernelArg(nearnest_neighbor_kernel, 5, sizeof(cl_int), &C); // 커널 함수 인자 값 전달 (버퍼와 커널 연결)
	status = clSetKernelArg(nearnest_neighbor_kernel, 6, sizeof(cl_int), &H); // 커널 함수 인자 값 전달 (버퍼와 커널 연결)
	status = clSetKernelArg(nearnest_neighbor_kernel, 7, sizeof(cl_int), &W); // 커널 함수 인자 값 전달 (버퍼와 커널 연결)
	status = clSetKernelArg(nearnest_neighbor_kernel, 8, sizeof(cl_int), &P); // 커널 함수 인자 값 전달 (버퍼와 커널 연결)
	status = clSetKernelArg(nearnest_neighbor_kernel, 9, sizeof(cl_int), &Q); // 커널 함수 인자 값 전달 (버퍼와 커널 연결)
	status = clSetKernelArg(nearnest_neighbor_kernel, 10, sizeof(cl_int), &num_elements); // 커널 함수 인자 값 전달 (버퍼와 커널 연결)
	...
	status = clEnqueueNDRangeKernel(cmdQueue, nearnest_neighbor_kernel, 1, NULL, globalWorkSize, NULL, 0, NULL, &event); // 커널 실행
	...
	//printData(input, N, C, H, W);
	//printData(output, N, C, P, Q);
	tofile(input, N * C * H * W, "../Validation_py/Input_C_un");
	tofile(output, N * C * P * Q, "../Validation_py/Output_C_un");

	// python 검증 스크립트 수행
	printf("\n *Validation with python \n");
	std::string command = "python ../Validation_py/upsample.py --N=" + std::to_string(N) + " --C=" + std::to_string(C) + " --H=" + std::to_string(H) 
	...
	clReleaseKernel(nearnest_neighbor_kernel);
