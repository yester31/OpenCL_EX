...
// dur_time(openCL) = 7.55217[msec]
// dur_time(cpu) = 1109.63196[msec]
...
int main() {
	...
	char* programSource = readSource("MatMul4.cl"); // 커널 함수 파일 로드
	...
	kernel = clCreateKernel(program, "register_blocking_matMul_kernel", &status); // 커널 생성 (커널 함수 이름을 인자로 전달)
