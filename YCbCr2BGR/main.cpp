...
int main()
{
	...
	input = (float*)malloc(sizeof(float)* N * C * H * W);		// input [N, C, H, W] YCbCr
	output = (float*)malloc(sizeof(float)* N * C * H * W);		// Ouput [N, H, W, C] BGR

	fromfile(input, N * C * H * W, "../Validation_py/Output_C_2");
	...
	char* programSource = readSource("ycbcr2bgr.cl"); // 커널 함수 파일 로드
	...
	cl_kernel ycbcr2bgr_kernel = NULL;
	// NCHW -> NHWC
	ycbcr2bgr_kernel = clCreateKernel(program, "ycbcr2bgr_kernel", &status); // 커널 생성 (커널 함수 이름을 인자로 전달)
	...
	status = clSetKernelArg(ycbcr2bgr_kernel, 0, sizeof(cl_mem), &buffer_O); // output
	status = clSetKernelArg(ycbcr2bgr_kernel, 1, sizeof(cl_mem), &buffer_I); // input
	status = clSetKernelArg(ycbcr2bgr_kernel, 2, sizeof(cl_int), &N); // 커널 함수 인자 값 전달 (버퍼와 커널 연결)
	status = clSetKernelArg(ycbcr2bgr_kernel, 3, sizeof(cl_int), &C); // 커널 함수 인자 값 전달 (버퍼와 커널 연결)
	status = clSetKernelArg(ycbcr2bgr_kernel, 4, sizeof(cl_int), &H); // 커널 함수 인자 값 전달 (버퍼와 커널 연결)
	status = clSetKernelArg(ycbcr2bgr_kernel, 5, sizeof(cl_int), &W); // 커널 함수 인자 값 전달 (버퍼와 커널 연결)
	status = clSetKernelArg(ycbcr2bgr_kernel, 6, sizeof(cl_int), &num_elements); // 커널 함수 인자 값 전달 (버퍼와 커널 연결)
	...
	status = clEnqueueNDRangeKernel(cmdQueue, ycbcr2bgr_kernel, 1, NULL, globalWorkSize, NULL, 0, NULL, &event); // 커널 실행
	...
	//tofile(input, N * C * H * W, "../Validation_py/Input_C_3");
	tofile(output, N * C * H * W, "../Validation_py/Output_C_3");

	// python 검증 스크립트 수행
	printf("\n *Validation with python \n");
	std::string command = "python ../Validation_py/ycbcr2bgr.py --N=" + std::to_string(N) + " --C=" + std::to_string(C) + " --H=" + std::to_string(H) + " --W=" + std::to_string(W);
	...
	clReleaseKernel(ycbcr2bgr_kernel);
