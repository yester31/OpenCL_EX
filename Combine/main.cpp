...
int main()
{
	...
	input = (float*)malloc(sizeof(float)* N * 1 * H * W);		// input [N, 1, H, W] Y
	output = (float*)malloc(sizeof(float)* N * C * H * W);		// Ouput [N, C, H, W] YCbCr

	fromfile(input, N * 1 * H * W, "../Validation_py/Output_C_4");
	fromfile(output, N * C * H * W, "../Validation_py/Output_C_2");
	...
	buffer_I = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(float) * N * 1 * H * W, NULL, &status);		// 디바이스 버퍼 객체 생성(입력용)
	buffer_O = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(float) * N * C * H * W, NULL, &status);	// 디바이스 버퍼 객체 생성(출력용)
	...
	status = clEnqueueWriteBuffer(cmdQueue, buffer_I, CL_FALSE, 0, sizeof(float) * N * 1 * H * W, input, 0, NULL, NULL); // host (input) -> device (buffer_I)전달
	status = clEnqueueWriteBuffer(cmdQueue, buffer_O, CL_FALSE, 0, sizeof(float) * N * C * H * W, output, 0, NULL, NULL); // host (input) -> device (buffer_I)전달
	...
	char* programSource = readSource("Combine.cl"); // 커널 함수 파일 로드
	...
	cl_kernel Combine_channel_kernel = NULL;
	// NCHW -> NHWC
	Combine_channel_kernel = clCreateKernel(program, "combine_first_channel_kernel", &status); // 커널 생성 (커널 함수 이름을 인자로 전달)
	...
	status = clSetKernelArg(Combine_channel_kernel, 0, sizeof(cl_mem), &buffer_O); // output
	status = clSetKernelArg(Combine_channel_kernel, 1, sizeof(cl_mem), &buffer_I); // input
	status = clSetKernelArg(Combine_channel_kernel, 2, sizeof(cl_int), &num_elements); // 커널 함수 인자 값 전달 (버퍼와 커널 연결)
	...
	status = clEnqueueNDRangeKernel(cmdQueue, Combine_channel_kernel, 1, NULL, globalWorkSize, NULL, 0, NULL, &event); // 커널 실행
	...
	//tofile(input, N * C * H * W, "../Validation_py/Input_C_4");
	tofile(output, N * C * H * W, "../Validation_py/Output_C_5");

	// python 검증 스크립트 수행
	printf("\n *Validation with python \n");
	std::string command = "python ../Validation_py/combine_channel.py --N=" + std::to_string(N) + " --C=" + std::to_string(C) + " --H=" + std::to_string(H) + " --W=" + std::to_string(W);
	...
	clReleaseKernel(Combine_channel_kernel);
